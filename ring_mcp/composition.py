...
class MCPServerComposition:
    ...
    def _register_tools(self, app: FastMCP) -> None:
        ...
        @app.tool(name="list_connected_servers", description="List all connected MCP servers and their namespaces")
        async def list_connected_servers() -> list[dict[str, Any]]:
            """List all connected MCP servers and their namespaces."""
            return [
                {"namespace": namespace, "server_url": client.base_url, "connected": client.connected}
                for namespace, client in self.connected_servers.items()
            ]

        @app.tool(name="connect_server", description="Connect to another MCP server")
        ...
        async def disconnect_server(namespace: str) -> dict[str, Any]:
            ...
            try:
                ...
                return {"status": "disconnected", "namespace": namespace, "tools_removed": tools_to_remove}
        ...
        async def proxy_tool_call(tool_name: str, **kwargs) -> Any:

...
export function Dashboard() {
    ...
    const deviceCount = health?.health_status?.devices_accessible ?? 0;

    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight text-white">Ring MCP Dashboard</h2>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <Card className="border-slate-800 bg-slate-950/50">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        ...
                        <Shield className={`h-4 w-4 ${backendOk ? "text-emerald-500" : "text-slate-500"}`} />
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold text-white">
                            {isLoading ? "…" : isError ? "Disconnected" : backendOk ? "Connected" : "No auth"}
                        </div>
                        <p className="text-xs text-slate-400">
                            {backendOk ? `${deviceCount} device(s)` : "Configure Ring in Settings"}
                        </p>
                    </CardContent>
                </Card>

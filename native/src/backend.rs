...
const BACKEND_NAME: &str = "ring-mcp-backend.exe";
const BACKEND_PORT: u16 = 10729;
const BACKEND_TAG: &str = "ring-mcp-backend-x86_64-pc-windows-msvc.exe";
const ENV_PORT: &str = "PORT";
const ENV_HOST: &str = "HOST";
const ENV_TAURI: &str = "RING_MCP_TAURI";
...
fn free_port(port: u16) -> bool {
    #[cfg(windows)]
    {
        let img_kill = "Stop-Process -Name 'ring-mcp-backend' -Force -ErrorAction SilentlyContinue; \
                        Stop-Process -Name 'ring-mcp-native' -Force -ErrorAction SilentlyContinue; \
                        taskkill /F /IM ring-mcp-backend.exe /T 2>`$null; \
                        taskkill /F /IM ring-mcp-native.exe /T 2>`$null";
            ...
            if i == 15 {
                let elevated = format!(
                    "Start-Process powershell -Verb RunAs -WindowStyle Hidden -ArgumentList \
                     '-NoProfile -Command \"Stop-Process -Name ring-mcp-backend -Force -ErrorAction SilentlyContinue; \
                     taskkill /F /IM ring-mcp-backend.exe /T 2>`$null; \
...
fn watch_backend_stream<R: std::io::Read + Send + 'static>(stream: R, app: AppHandle, ready_marker: &'static str) {
...
pub fn spawn_backend(app: AppHandle, state: &BackendProcess) -> Result<String, String> {
    ...
    log_line(
        &app,
        &format!("spawning {} (cwd {}) on port 10729",
    ...
    // Watch stdout/stderr for backend-ready signal
    if let Some(out) = stdout {
        let handle = app.clone();
        thread::spawn(move || watch_backend_stream(out, handle, "Uvicorn running on"));
    }
    if let Some(err) = stderr {
        let handle = app.clone();
        thread::spawn(move || watch_backend_stream(err, handle, "Uvicorn running on"));
    }
